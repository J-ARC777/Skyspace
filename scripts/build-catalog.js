...
// B-V color index → RGB (more accurate than spectral class approximation)
function colorFromBV(bv) {
  if (isNaN(bv)) return SPECTRAL_COLORS.default;
  const t = Math.max(-0.4, Math.min(2.0, bv));
  let r, g, b;
  if (t < 0.0) {
    r = 0.63 + t * 0.28; g = 0.71 + t * 0.14; b = 1.00;
  } else if (t < 0.4) {
    r = 0.63 + t * 0.68; g = 0.71 + t * 0.56; b = 1.00 - t * 0.28;
  } else if (t < 0.8) {
    r = 0.90 + (t - 0.4) * 0.25; g = 0.93 - (t - 0.4) * 0.08; b = 0.89 - (t - 0.4) * 0.55;
  } else if (t < 1.4) {
    r = 1.00; g = 0.90 - (t - 0.8) * 0.24; b = 0.67 - (t - 0.8) * 0.52;
  } else {
    r = 1.00; g = 0.76 - (t - 1.4) * 0.26; b = Math.max(0.15, 0.36 - (t - 1.4) * 0.35);
  }
  return [
...
async function buildCatalog() {
  // HYG v3 — combines Hipparcos, Yale Bright Star, and Gliese catalogs.
  // Hosted on GitHub CDN; already has heliocentric Cartesian coords and B-V index.
  const urls = [
  ...
  if (!text) {
    console.log('All sources failed — generating synthetic catalog.');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (isNaN(mag) || mag > 8.0) continue;

    const dist = parseFloat(c[iDist]);
    if (isNaN(dist) || dist <= 0 || dist > 2000) continue;
    ...
    const hip    = parseInt(c[iHip]) || 0;
    const proper = c[iProper]?.trim() || '';
    const bf     = c[iBF]?.trim() || '';
    const con    = c[iCon]?.trim() || '';
    const spect  = c[iSpect]?.trim() || '';
    const ci     = parseFloat(c[iCI]);

    // Prefer proper name, fall back to Bayer/Flamsteed + constellation
    const displayName = proper || (bf && con ? `${bf} ${con}` : '');

    let nameIdx = -1;
    if (displayName) {
      if (nameMap[displayName] === undefined) { nameMap[displayName] = names.length; names.push(displayName); }
      nameIdx = nameMap[displayName];
    }

    const color = isNaN(ci) ? spectralColor(spect) : colorFromBV(ci);
    validStars.push({ hip, vmag: mag, pos: { x, y, z, distLy: dist * 3.26156 }, color, nameIdx });
  }

  console.log(`Parsed ${validStars.length} valid stars, ${names.length} named`);
  writeBinary(validStars, names, 48);
}
